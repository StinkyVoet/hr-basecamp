...
if __name__ == "__main__":
    # How many unique days have equal average temperatures in March 1995 and March 2010?
    total_days = temperatures[0][2] + temperatures[1][2]
    print(len(total_days) - len(set(total_days)))

    # How many unique days have equal average temperatures in March 1995 and March 2020?
    total_days = temperatures[0][2] + temperatures[2][2]
    print(len(total_days) - len(set(total_days)))

    # Which year has a day with highest temprature in March?
    highest_temp = (0, 0)
    for year in temperatures:
        year_max = float(max(year[2]))
        if year_max > highest_temp[1]:
            highest_temp = (year[0], year_max)
    print(highest_temp[0])

    # Which year had the warmest March?
    highest_temp = (0, 0)
    for year in temperatures:
        average = float(sum(map(float, year[2])) / len(year[2]))
        if average > highest_temp[1]:
            highest_temp = (year[0], average)
    print(highest_temp[0])
